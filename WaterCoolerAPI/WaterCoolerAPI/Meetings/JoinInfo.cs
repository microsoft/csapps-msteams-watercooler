...
namespace WaterCoolerAPI.Meetings
{
    using System;
    using System.IO;
    using System.Net;
    ...
    public class JoinInfo
    {
        ...
        public static (ChatInfo, MeetingInfo) ParseJoinURL(string joinURL)
        {
            ...
            var regex = new Regex(Common.Constants.MeetingUrlRegex);
            var match = regex.Match(decodedURL);
            if (!match.Success)
            {
                throw new ArgumentException($"{Common.Constants.JoinURLCannotBeParsedException} {joinURL}.", nameof(joinURL));
            }

            using var stream = new MemoryStream(Encoding.UTF8.GetBytes(match.Groups[Common.Constants.Context].Value));
            var ctxt = (Context)new DataContractJsonSerializer(typeof(Context)).ReadObject(stream);
            var chatInfo = new ChatInfo
            {
                ThreadId = match.Groups[Common.Constants.Thread].Value,
                MessageId = match.Groups[Common.Constants.Message].Value,
